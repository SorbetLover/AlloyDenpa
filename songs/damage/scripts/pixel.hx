...
function onNoteCreation(event) {
	...
	if (event.note.isSustainNote) {
		note.loadGraphic(Paths.image('stages/monoschool/ui/arrowEnds'), true, 7, 6);
		...
		note.loadGraphic(Paths.image('stages/monoschool/ui/arrows-pixels'), true, 17, 17);
...
function onStrumCreation(event) {
	...
	strum.loadGraphic(Paths.image('stages/monoschool/ui/arrows-pixels'), true, 17, 17);
...
function onCountdown(event) {
	...
	event.spritePath = switch(event.swagCounter) {
		case 0: null;
		case 1: 'stages/monoschool/ui/ready';
		case 2: 'stages/monoschool/ui/set';
		case 3: 'stages/monoschool/ui/go';
	};
}

function onPlayerHit(event:NoteHitEvent) {
	if (!enablePixelUI) return;
	event.ratingPrefix = "stages/monoschool/ui/";
