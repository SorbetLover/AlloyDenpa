...
function create() {
	...
	if(SONG.strumLines[0].keyCount == 4){
		if (mania == 4 || mania == 0 || mania == null) {
		disableScript();
		}
	} else {
		mania = SONG.strumLines[0].keyCount;
	}
	if(PlayState.instance.curSong == "vespers"){
		disableScript();
		trace("FUCK TGHIS SHIT. STOP CRASHING BECOUSE OF A NULL VALUE I DONT FUCKIMG KNOW HOW TO FIX ITS THE 3RD CHART I TRY FUCK YOu");
	}
